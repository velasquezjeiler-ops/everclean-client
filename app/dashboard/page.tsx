...
const SERVICE_LABELS: Record<string, string> = {
  ...
  OFFICE_CLEANING: 'Office Cleaning', COMMERCIAL_CLEANING: 'Commercial Cleaning',
  POST_CONSTRUCTION: 'Post Construction', MEDICAL_CLEANING: 'Medical / Clinic',
  APARTMENT_CLEANING: 'Apartment Cleaning', MAID_SERVICES: 'Maid Services',
  SAME_DAY_CLEANING: 'Same Day Cleaning', RESIDENTIAL: 'Residential Cleaning',
  STANDARD_CLEANING: 'Standard Cleaning', ONE_TIME: 'One-Time Cleaning',
  ONE_TIME_CLEANING: 'One-Time Cleaning', ROOM_CLEANING: 'Room Cleaning',
  HOUSEKEEPING: 'Housekeeping', WINDOW_CLEANING: 'Window Cleaning',
  CARPET_CLEANING: 'Carpet Cleaning',
};

function formatServiceType(raw: string): string {
  if (!raw) return 'Cleaning Service';
  return SERVICE_LABELS[raw] || raw.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, (c: string) => c.toUpperCase());
}
...
export default function ClientDashboard() {
  ...
  function BookingCard({ b, showActions = true }: { b: any; showActions?: boolean }) {
    ...
    const svcLabel = formatServiceType(b.serviceType);
      ...
      {ratingModal && (
        <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-50 p-4 sm:items-center">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-500 mb-4">{formatServiceType(ratingModal.serviceType)}</p>
      ...
      {claimModal && (
        <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-50 p-4 sm:items-center">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-500 mb-4">{formatServiceType(claimModal.serviceType)} · {claimModal.city}</p>
                ...
                <div key={i} className="bg-white rounded-2xl border border-gray-200 p-5">
                  ...
                  <div className="text-xs text-gray-500 mb-4">
                    Last service: {pro.lastBooking?.scheduledAt
                      ...
                      : '—'}
                    {' · '}{formatServiceType(pro.lastBooking?.serviceType || '')}
                  </div>
            ...
            <div className="space-y-3">
              {completed.sort((a, b) => new Date(b.scheduledAt).getTime() - new Date(a.scheduledAt).getTime()).map(b => (
                <div key={b.id} className="bg-white rounded-2xl border border-gray-200 p-5">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <p className="font-semibold text-gray-900">{formatServiceType(b.serviceType)}</p>
                      <p className="text-xs text-gray-500">{b.address}{b.city ? ', ' + b.city : ''}</p>
                    </div>
                    {b.totalAmount && (
