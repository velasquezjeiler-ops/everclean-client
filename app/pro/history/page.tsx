...
export default function ProHistory() {
  ...
  const stats = useMemo(() => {
    ...
    const avgRating = rated.length
      ? (rated.reduce((s, j) => s + Number(j.rating || 0), 0) / rated.length).toFixed(1)
      : '5.0';
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="w-9 h-9 border-[3px] border-[#E2E8F0] border-t-[#1565C0] rounded-full animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div style={{ width: '100%', maxWidth: 1360 }}>
      <div style={{ marginBottom: 18 }}>
        <h1 style={{ fontSize: 32, fontWeight: 800, color: C.text, lineHeight: 1.05, marginBottom: 4 }}>
          {t('sidebar.history')}
        </h1>
        <p style={{ fontSize: 14, color: C.muted }}>
          Review completed jobs with the same workflow view as My Jobs.
        </p>
      </div>
      ...
      {jobs.length === 0 ? (
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <div style={{ fontSize: 24, fontWeight: 800, color: C.text }}>Completed Jobs</div>
            <div
              style={{
                minWidth: 30,
                height: 28,
