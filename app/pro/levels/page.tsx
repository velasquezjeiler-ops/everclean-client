  ...
  return (
    <div style={{maxWidth:900,margin:'0 auto',fontFamily:"'Inter',system-ui,sans-serif"}}>
      <div style={{marginBottom:28}}>
        ...
        <h1 style={{margin:0,fontSize:'clamp(22px,3vw,32px)',fontWeight:600,color:C.ink}}>My Level & Benefits</h1>
        <p style={{margin:'6px 0 0',color:C.muted,fontSize:14}}>More services = higher level = more income + better benefits.</p>
      </div>
      ...
      <div style={{...card({padding:28,marginBottom:16}),background:`linear-gradient(135deg,${currentLevel.color}22,${currentLevel.bg})`}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:16}}>
          <div style={{display:'flex',alignItems:'center',gap:16}}>
            <span style={{fontSize:52}}>{currentLevel.icon}</span>
            <div>
              <div style={{fontSize:11,fontWeight:700,color:C.muted,textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:4}}>{currentLevel.label} PROFESSIONAL</div>
          ...
          <div style={{textAlign:'right'}}>
            ...
            <div style={{fontSize:12,color:C.muted,marginTop:4}}>Platform fee: {currentLevel.commission}%</div>
          </div>
        </div>
        {nextLevel&&(
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        <div style={card({padding:24})}>
          <h3 style={{margin:'0 0 16px',fontSize:15,fontWeight:600,color:C.ink}}>⭐ Points System</h3>
        ...
        <div style={card({padding:24})}>
          <h3 style={{margin:'0 0 16px',fontSize:15,fontWeight:600,color:C.ink}}>📋 Recent Activity</h3>
          {events.length===0
