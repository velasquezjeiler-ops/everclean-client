...
export default function ProPayments() {
  ...
  const tier =
    hourlyRate <= 18
      ? { label: t('pro.payments.priority'), bg: '#D1FAE5', color: C.greenDk }
      : hourlyRate <= 20
        ? { label: t('pro.payments.tier2'), bg: '#FEF3C7', color: '#92400E' }
        : { label: t('pro.payments.open'), bg: '#FEF3C7', color: '#92400E' };
  ...
  return (
    <div style={{ width: '100%', fontFamily: 'Poppins, sans-serif' }}>
      ...
      <div className="payments-page">
        <h1 style={{ fontSize: 22, fontWeight: 700, color: C.text, margin: '0 0 20px' }}>{t('pro.payments.title')}</h1>
          ...
          <div className="payments-column">
            <section style={card}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 16 }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 800, color: C.text }}>{t('pro.payments.directDeposit')}</div>
                  <div style={{ fontSize: 12, color: C.muted, marginTop: 3 }}>
                    {t('pro.payments.connectBankText')}
                  </div>
                </div>
                <span
                  style={{
                ...
                >
                  {isConnected ? t('pro.payments.connected') : t('pro.payments.notConnected')}
                </span>
              </div>

              {isConnected ? (
                <div style={{ padding: '14px 16px', background: C.bg, borderRadius: 12 }}>
                  <div style={{ fontSize: 13, color: C.text, fontWeight: 700 }}>{t('pro.payments.bankConnected')}</div>
                  <div style={{ fontSize: 12, color: C.muted, marginTop: 3 }}>
                    {t('pro.payments.payoutFriday')}
                  </div>
                </div>
              ) : (
                <button
                ...
                >
                  {onboarding ? t('pro.payments.redirecting') : t('pro.payments.connectBank')}
                </button>
              )}
            </section>
            ...
            <section style={card}>
                ...
                <div style={{ position: 'relative', zIndex: 1 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 18 }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: 9, textTransform: 'uppercase' }}>
                        {t('pro.payments.status')}
                      </div>
                      <div style={{ color: canRequestCard ? C.green : C.warning, fontSize: 12, fontWeight: 800 }}>
                        {canRequestCard ? t('pro.payments.eligible') : t('pro.payments.pending')}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div style={{ marginTop: 14 }}>
                {canRequestCard ? (
                  ...
                  >
                    {t('pro.payments.requestCard')}
                  </button>
                ) : (
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                      <span style={{ fontSize: 12, color: '#92400E', fontWeight: 800 }}>
                        {completedJobs}/3 {t('pro.payments.servicesCompleted')}
                      </span>
                      <span style={{ fontSize: 12, color: C.muted }}>
                        {Math.max(3 - completedJobs, 0)} {t('common.remaining')}
                      </span>
                    </div>
          ...
          <div className="payments-column">
            <section style={card}>
              ...
              <div className="payments-rate-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div style={{ padding: '18px 16px', background: C.bg, borderRadius: 12, textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>{t('pro.payments.perHour')}</div>
                </div>
                <div style={{ padding: '18px 16px', background: tier.bg, borderRadius: 12, textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>{t('pro.payments.auctionTier')}</div>
                </div>
              </div>
              <div style={{ marginTop: 12, fontSize: 12, color: C.muted }}>
                {t('pro.payments.ratePriority')}{' '}
                <Link href="/pro/profile" style={{ color: C.blue, fontWeight: 700 }}>{t('pro.payments.profile')}</Link>
                .
              </div>
            </section>
                  ...
                  return (
                    ...
                    >
                      <div style={{ fontSize: 13, fontWeight: 800, color: active ? C.blue : C.text }}>
                        {s === 'WEEKLY' ? t('pro.payments.weekly') : t('pro.payments.biWeekly')}
                      </div>
                      <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>
                        {s === 'WEEKLY' ? t('pro.payments.everyFriday') : t('pro.payments.everyTwoWeeks')}
                      </div>
                    </div>
                  );
                })}
            ...
            <section style={{ ...card, background: `linear-gradient(135deg, ${C.navy}, ${C.blue})`, color: '#fff' }}>
              <div style={{ fontSize: 12, fontWeight: 800, opacity: 0.72, textTransform: 'uppercase', letterSpacing: 0.8 }}>
                {t('pro.payments.paymentReadiness')}
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 14 }}>
                <div style={{ background: 'rgba(255,255,255,0.12)', borderRadius: 12, padding: 12 }}>
                  <div style={{ fontSize: 22, fontWeight: 900 }}>{isConnected ? t('pro.payments.on') : t('pro.payments.off')}</div>
                  <div style={{ fontSize: 11, opacity: 0.72 }}>{t('pro.payments.directDepositShort')}</div>
                </div>
                <div style={{ background: 'rgba(255,255,255,0.12)', borderRadius: 12, padding: 12 }}>
                  <div style={{ fontSize: 22, fontWeight: 900 }}>{completedJobs}</div>
                  <div style={{ fontSize: 11, opacity: 0.72 }}>{t('pro.payments.completedJobs')}</div>
                </div>
              </div>
            </section>
          </div>
