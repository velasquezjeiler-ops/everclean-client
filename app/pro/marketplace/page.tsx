    ...
    <div style={{ width: '100%', fontFamily: font }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ color: C.muted, fontSize: 13, margin: 0 }}>
          {t('pro.marketplace.rateLine')}: <strong style={{ color: C.navy }}>${proRate}/{t('units.hour')}</strong> Â· {t('pro.marketplace.auctionLine')}
        </p>
      </div>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 18, fontWeight: 600 }}>{tierLabel} - ${proRate}/{t('units.hour')}</div>
      ...
      {jobs.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 20px', background: '#fff', borderRadius: 14, border: `1px solid ${C.border}` }}>
          <div style={{ width: 58, height: 58, borderRadius: '50%', background: C.bg, color: C.blue, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontWeight: 600, fontSize: 20, marginBottom: 12 }}>0</div>
          <div style={{ fontWeight: 600, fontSize: 16, color: C.text, marginBottom: 6 }}>{t('pro.marketplace.noJobsTitle')}</div>
          ...
          {proRate > 18 && (
            <div style={{ marginTop: 16, padding: '10px 16px', background: `${C.warning}15`, border: `1px solid ${C.warning}30`, borderRadius: 8, fontSize: 12, color: '#92400E', maxWidth: 360, marginLeft: 'auto', marginRight: 'auto' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {jobs.map((job, idx) => {
            ...
            return (
              <div key={job.id} style={{ background: '#fff', borderRadius: 14, border: `1px solid ${C.border}`, overflow: 'hidden', boxShadow: C.shadow, animation: `fadeIn 0.3s ease ${idx * 0.05}s both` }}>

                <div style={{ padding: '14px 16px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10, marginBottom: 10 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                      <div style={{ width: 44, height: 44, background: `${C.navy}10`, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 600, color: C.navy, flexShrink: 0 }}>
                        {SVC_MARK[job.service_type] || 'EC'}
                      </div>
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontWeight: 600, fontSize: 14, color: C.text }}>{serviceLabel(t, job.service_type)}</div>
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      <div style={{ fontSize: 20, fontWeight: 600, color: C.greenDk }}>+${Number(job.estimated_payout || 0).toFixed(0)}</div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
                    {date && (
                      <span style={{ background: C.bg, color: C.muted, padding: '3px 9px', borderRadius: 9999, fontSize: 10 }}>
                        {date.toLocaleDateString(locale, { month: 'short', day: 'numeric' })} Â· {date.toLocaleTimeString(locale, { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    )}
                    ...
                    {job.sqft && <span style={{ background: C.bg, color: C.muted, padding: '3px 9px', borderRadius: 9999, fontSize: 10 }}>{job.sqft} {t('units.sqft')}</span>}
                    {job.frequency && job.frequency !== 'ONE_TIME' && <span style={{ background: '#FEF3C7', color: '#92400E', padding: '3px 9px', borderRadius: 9999, fontSize: 10, fontWeight: 600 }}>{serviceLabel(t, job.frequency)}</span>}
                    <span style={{ background: C.bg, color: C.muted, padding: '3px 9px', borderRadius: 9999, fontSize: 10 }}>{fill(t('pro.marketplace.minutesAgo'), { minutes: mins })}</span>
                    {job.distance_miles && (
                  ...
                  {isClaimed ? (
                    ...
                    <button onClick={() => claimJob(job.id)} disabled={claiming === job.id} style={{ width: '100%', padding: '12px 0', borderRadius: 8, border: 'none', cursor: 'pointer', background: claiming === job.id ? C.muted : C.green, color: '#fff', fontSize: 13, fontWeight: 600, boxShadow: 'none', transition: 'all 0.2s' }}>
